    ...
    protected function pArray(
        ...
        foreach ($diff as $i => $diffElem) {
            ...
            if ($diffType === DiffElem::TYPE_KEEP || $diffType === DiffElem::TYPE_REPLACE) {
                ...
                if (!empty($delaycantidadd)) {
                    /** @var Node $delaycantidaddNode */
                    foreach ($delaycantidadd as $delaycantidaddNode) {
                        if ($insertNewline) {
                            $delaycantidaddComments = $delaycantidaddNode->getComments();
                            if ($delaycantidaddComments) {
                                $result .= $this->pComments($delaycantidaddComments) . $this->nl;
                            }
                        }

                        $this->safeAppend($result, $this->p($delaycantidaddNode, true));
        ...
        if (!empty($delaycantidadd)) {
            ...
            foreach ($delaycantidadd as $delaycantidaddNode) {
                ...
                $result .= $this->p($delaycantidaddNode, true);
                $first = false;
            }
            $result .= $extraRight;
        }
